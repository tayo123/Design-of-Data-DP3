...
tip = d3.tip()
  ...
  .direction('n')
  .html(function(d) {
    return nameById.get(d.id) + "<br/>Change in Perception: " + (rateById.get(d.id)*100).toFixed(2) + "%" +
    "<br/>Climate change Perception: " + (popById.get(d.id)*100).toFixed(2) + "%"
 });
...
queue()
  ...
  .defer(d3.tsv, "county_growth_all.tsv", function (d) {
    for (var propertyName in d) {
      if (propertyName == "Area" || propertyName == "key") {
        continue;
      };
      d[propertyName] = +d[propertyName];
    }

    filterTSV(d, "futuregen");
  }).await(ready);

function ready(error, us) {
  d3.select('#categories').on("change", function() {
    // remove bars to reset 
    bars.remove().exit();
...
function filterTSV(data, category) {
  if (data.key == category) {
    nation.add([data]);
    extant.push(data.id);

    rateById.set(data.id, data.Per_capita_personal_income);
    popById.set(data.id, data.Population);
    nameById.set(data.id, data.Area);
    min = Math.min(min,data.Per_capita_personal_income);
    max = Math.max(max,data.Per_capita_personal_income);
  }
}
